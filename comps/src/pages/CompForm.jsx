...
export default function CompForm({ pool, comp, theme, onToggleTheme, onBack, onSaved }) {
  const isEdit = !!comp
  const [form, setForm] = useState(() => {
    if (isEdit) {
      const f = {}
      Object.keys(EMPTY).forEach(k => { f[k] = comp[k] ?? EMPTY[k] })
      return f
    }
    return { ...EMPTY }
  })
  ...
  async function submit(e) {
    ...
    let result
    if (isEdit) {
      result = await supabase.from('comps').update(payload).eq('id', comp.id).select().single()
    } else {
      result = await supabase.from('comps').insert(payload).select().single()
    }

    // bump pool updated_at
  ...
  return (
    <div>
      <Header
        title={isEdit ? 'Edit Comp' : 'Add Comp'}
        eyebrow={pool.name}
        theme={theme}
        ...
        <form onSubmit={submit}>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.checkRow}>
              ...
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={form.over_ask}
                  onChange={e => set('over_ask', e.target.checked)}
                />
                Sold Over Ask
              </label>
            </div>
          ...
          <div className={styles.actions}>
            <button type="submit" className={styles.submit} disabled={loading}>
              {loading ? 'Saving…' : isEdit ? 'Save Changes' : 'Add Comp'}
            </button>
